document.addEventListener('DOMContentLoaded', () => {
    ...
    if (projectsGrid && modal && modalBody && modalCloseButton) {
        ...
        projects.forEach((project, index) => {
            ...
            card.className = 'bg-white rounded-lg flex flex-col justify-between border border-gray-200/80 hover:border-brand-green transition-all duration-300 shadow-lg hover:shadow-xl';
            ...
            card.innerHTML = `
                <div class="p-5 flex-grow">
                    <h3 class="text-xl font-bold text-dark-text mb-3">${project.name}</h3>
                    <p class="text-medium-text mb-4">${project.description}</p>
                </div>
                <div class="p-5 pt-0">
        ...
        const openModal = (project) => {
            let detailsHtml = `
                <h2 class="text-3xl font-extrabold text-dark-text mb-2">${project.name}</h2>
